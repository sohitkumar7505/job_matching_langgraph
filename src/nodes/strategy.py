...
def strategy_node(state):
    ...
    sorted_approved = sorted(
        approved_jobs, key=lambda x: x.get("score", 0), reverse=True
    )
        ...
        "final_output": {
            **validated_strategy.dict(),
            "stats": {
                ...
                "average_approved_score": (
                    sum(j.get("score", 0) for j in approved_jobs) / len(approved_jobs)
                    if approved_jobs
                    else 0
                ),
                "top_match": sorted_approved[0]["title"] if sorted_approved else None,
                "top_match_score": (
                    sorted_approved[0].get("score", 0) if sorted_approved else 0
                )
            }
        }
    }
